import { clamp, normalizeSpaces } from "../core/utils.js";
...
export class HintInjector {
  ...
  wrapFirstMatch(container, rule) {
    ...
    const target = exactContainer || textContainers.find((item) => normalizeSpaces(item.textContent).includes(normalizeSpaces(rule.text)));
    if (!target) return false;
    const walker = document.createTreeWalker(target, NodeFilter.SHOW_TEXT, {
      acceptNode: (node) => node.nodeValue.includes(rule.text) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT
    });
  ...
  bindTooltips(container) {
    this.scrollContainer = container.closest("[data-reading-area]");
  ...
  repositionTooltip() {
    ...
    const gap = 10;
    const maxWidth = Math.max(180, Math.min(window.innerWidth - margin * 2, window.innerWidth <= 768 ? window.innerWidth * 0.82 : 460));
