...
class AudioManager {
  ...
  setupEventListeners() {
    ...
    if (this.audioProgressContainer) {
      this.setupDraggableSlider(this.audioProgressContainer, (percent) => {
        if (this.activeAudio && this.activeAudio.duration) {
          this.activeAudio.currentTime = percent * this.activeAudio.duration;
          this.updateProgress();
        }
      });
    }
    ...
    if (this.audioVolumeContainer) {
      this.setupDraggableSlider(this.audioVolumeContainer, (percent) => {
        this.setVolumeFromPercent(percent);
      });
    }
  ...
  setupDraggableSlider(container, onUpdate) {
  ...
  setVolumeFromPercent(percent) {
    ...
    if (this.activeAudio) {
      this.activeAudio.volume = percent;
      this.activeAudio.muted = false;
    }

    if (this.audioVolumeSlider) {
      this.audioVolumeSlider.style.width = `${percent * 100}%`;
    }
  ...
  setVolume(event) {
    ...
    const percent = (event.clientX - rect.left) / rect.width;
    this.setVolumeFromPercent(Utils.clamp(percent, 0, 1));
  }
